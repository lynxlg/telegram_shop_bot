...
Создавай этот документ только после того, как sibling `feature.md` переведен в `status: active`. Для состояния `Plan Ready` и дальше instantiated `implementation-plan.md` должен иметь `status: active`; `draft` допустим только как локальный черновик до прохождения gate `Design Ready → Plan Ready`.
