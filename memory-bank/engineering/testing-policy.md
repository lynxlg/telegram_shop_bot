...
- **Test data pattern:** базовый reusable setup живёт в [tests/conftest.py](../../tests/conftest.py). Для Telegram объектов и ORM-сущностей используй существующие factory fixtures (`message_factory`, `callback_factory`, `category_factory`, `product_factory`) вместо дублирования hand-made setup в каждом тесте.
...
- **Mocks and monkeypatching:** для unit-сценариев, которые не требуют реальной БД, canonical pattern — `monkeypatch`, `AsyncMock`, `MagicMock`, `SimpleNamespace`, как в [tests/test_database_unit.py](../../tests/test_database_unit.py). Мокать нужно I/O boundary, а не внутренние assertions tested function.
- **Where to add tests:**
...
- **CI reality:** текущий GitHub Actions workflow в [ci.yml](../../.github/workflows/ci.yml) проверяет shell/tooling/markdown bootstrap, но не гоняет Python test suite. Поэтому для closure feature локальный прогон app-level тестов обязателен; отсутствие Python tests в CI нельзя трактовать как waiver.
