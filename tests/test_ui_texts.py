...
from app.services.order_text import format_active_orders_text, format_order_status
...
def test_order_status_text_builder_maps_known_and_unknown_statuses() -> None:
    orders = [
        ...
        SimpleNamespace(order_number="ORD-000002", status="assembling"),
        SimpleNamespace(order_number="ORD-000003", status="mystery"),
    ]

    assert format_order_status("new") == "Принят"
    assert format_order_status("mystery") == "Статус: mystery"
    assert format_active_orders_text(orders) == (
        "Активные заказы:\n\n"
        "1. ORD-000001 - Принят\n"
        "2. ORD-000002 - Собран\n"
        "3. ORD-000003 - Статус: mystery"
    )
